...
currency <- read.csv("D:/Sem1/BF/Project/USD_INR_Historical_Data.csv") ### 9 years data from 11/01/2010 (2370 total obs)
str(currency)
class(currency)
setDT(currency)
...
# 10/06/2020
acf(fitholt$residuals) # models is not adequate as residuals are correlated at 
# several legs
...
modelarima <- arima(train,order=c(4,1,1))
plot(modelarima$residuals)
summary(modelarima)
...
Box.test(modelarima$residuals,10,type='Ljung') # check the residuals Box-Ljung test 
Box.test(modelarima$residuals^2,10,type='Ljung') 
fcasta <- forecast(modelarima,h=780)
plot(fcasta)
lines(fitted(modelarima), col="yellow") ## plot the model
modelarima
fcasta
...
modelarima$residuals # model1 is arima model
acf(modelarima$residuals) # residuals of ARIMA model are not significantly correlated
# hence our model is adquate
